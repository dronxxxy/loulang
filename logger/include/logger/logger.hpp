...
namespace logger {
  ...
  public:
    template<typename ... Args> void error(std::string_view message, Args ...args) {
      this->log(Level::Error, std::vformat(message, std::make_format_args(args...)));
    }

    template<typename ... Args> void warning(std::string_view message, Args ...args) {
      this->log(Level::Warning, std::vformat(message, std::make_format_args(args...)));
    }

    template<typename ... Args> void info(std::string_view message, Args ...args) {
      this->log(Level::Info, std::vformat(message, std::make_format_args(args...)));
    }

    template<typename ... Args> void debug(std::string_view message, Args ...args) {
